        ...
        <?php if (!empty($userCourses)): ?>
            ...
            <div id="in-progress-section" class="mt-10">
                ...
                <div id="in-progress-courses-container" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    <!-- Cursos em andamento serão inseridos aqui -->
                </div>
            </div>
            ...
            <div id="finished-section" class="mt-12">
                ...
                <div id="finished-courses-container" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    <!-- Cursos finalizados serão inseridos aqui -->
                </div>
            </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // --- NOVO: Função para enviar a atualização para o servidor ---
    async function updateStatusOnServer(courseId, newStatus) {
        try {
            ...
            if (!result.success) {
                console.error('Falha ao salvar o progresso:', result.message);
                // Opcional: Reverter a alteração na UI se o servidor falhar
            }
        } catch (error) {
            console.error('Erro de rede ao salvar o progresso:', error);
        }
    }
    ...
    document.getElementById('dashboard-view').addEventListener('click', function(e) {
        ...
        let newStatus = course.user_status;
        if (action === 'finish') {
        ...
        // Atualiza o estado localmente primeiro para uma resposta rápida da UI
