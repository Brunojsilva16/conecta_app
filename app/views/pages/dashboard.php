    <!-- ATUALIZADO: O HTML foi simplificado para se encaixar no novo layout -->
    ...
    <!-- O Script para a lógica dinâmica permanece o mesmo -->
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function renderCourses() {
                ...
                const createCourseCard = (course) => {
                    ...
                    const imageUrl = course.image_url ? `<?= BASE_URL ?>${course.image_url}` : 'https://placehold.co/400x225/4338CA/FFFFFF?text=Curso';
                ...
                <div class="bg-white rounded-xl shadow-lg border border-slate-200 flex flex-col justify-between transition-transform hover:-translate-y-1 overflow-hidden">
                    <img src="${imageUrl}" alt="${course.title}" class="w-full h-40 object-cover">
                    <div class="p-6 flex flex-col flex-grow">
                        <div>
                            <h3 class="text-lg font-bold text-slate-900 mb-2 h-14">${course.title}</h3>
