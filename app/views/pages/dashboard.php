...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function renderCourses() {
        ...
        const createCourseCard = (course) => {
            ...
            // Lógica da URL da imagem corrigida
            const imageUrl = course.image_url 
                ? `<?= BASE_URL ?>${course.image_url}` 
                : '<?= BASE_URL . "/assets/img/default_course.svg" ?>';

            return `
                <div class="bg-white rounded-xl shadow-lg border border-slate-200 overflow-hidden flex flex-col justify-between transition-transform hover:-translate-y-1">
                    <img src="${imageUrl}" alt="Capa do curso ${course.title}" class="w-full h-40 object-cover">
                    <div class="p-6 flex flex-col flex-grow">
                        <h3 class="text-lg font-bold text-slate-900 mb-2">${course.title}</h3>
                        <p class="text-sm text-slate-500 mb-4">Instrutor: ${course.instructor}</p>
                        <span class="text-xs font-semibold px-2 py-1 rounded-full mb-4 inline-block self-start ${isFinished ? 'bg-green-100 text-green-800' : 'bg-blue-100 text-blue-800'}">
                            ${course.user_status}
                        </span>
                    
                        <div class="flex items-center space-x-2 mt-auto pt-4 border-t">
                            ${isFinished ? `
                            ...
                            ` : `
                                <a href="<?= BASE_URL ?>/curso/${course.id}" class="w-1/2 bg-blue-600 hover:bg-blue-700 text-white font-semibold py-2 px-4 rounded-md text-sm transition-colors text-center">Continuar</a>
