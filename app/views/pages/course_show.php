                    ...
                    <?php else : ?>
                        ...
                        <div class="mt-6">
                            <?php if (Auth::isLogged()) : ?>
                                <?php
                                $isPlanNone = $userPlan === 'none';
                                $isCoursePremiumOrPlatinum = in_array($course['category'], ['premium', 'platinum']);

                                if ($isPlanNone && $isCoursePremiumOrPlatinum) :
                                ?>
                                    <a href="<?= BASE_URL ?>/planos" class="w-full text-center bg-amber-500 border border-transparent rounded-md py-3 px-8 flex items-center justify-center text-base font-medium text-white hover:bg-amber-600 transition-colors">
                                        Adquirir Plano para Acessar
                                    </a>
                                    <p class="text-center text-sm text-gray-600 mt-2">Este curso requer um plano de assinatura para ser acessado.</p>
                                <?php else : ?>
